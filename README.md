...
This repository is currently a local platform preview. The launcher, game registry, room-code flow, live lobby, Docker image, privacy boundary, and restart-safe SQLite snapshots are implemented. 3s & 7s, Thirteen, Blackjack, Texas Hold'em, Five Card Draw, JUAN, Rotating Rummy, and Finders Makers are playable in Solo, Multiplayer, and private pass-the-device Hot Seat modes. Snap is playable in Solo and Multiplayer; simultaneous reactions are intentionally not offered in Hot Seat.
...
- 3s & 7s, Thirteen, Blackjack, Texas Hold'em, Five Card Draw, and Snap share Cardcade's rules-neutral standard 52-card deck. JUAN owns an original 108-card color/action deck, Rotating Rummy owns an original 108-card Route deck, and Finders Makers owns a hidden Piece board. They reuse only the Cardcade presentation and room behavior that fits their physical objects.
...
The game catalog in `server/src/game-catalog.js` is the canonical source for availability, modes, and player limits. See [docs/architecture.md](docs/architecture.md), [docs/snap.md](docs/snap.md), [docs/juan.md](docs/juan.md), [docs/rotating-rummy.md](docs/rotating-rummy.md), [docs/finders-makers.md](docs/finders-makers.md), and [docs/privacy-and-deployment.md](docs/privacy-and-deployment.md).
