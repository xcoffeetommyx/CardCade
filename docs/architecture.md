...
`shared/card-presentation.js` resolves clockwise player order into named table slots relative to the local viewer. The local player is always south; one opponent uses north, two opponents occupy west and north with east left empty, and three opponents use west, north, and east. Opponent hands contain one privacy-safe card-back element for every projected hidden card and hang that hand in the scene's own 3D space. The scene owns a single perspective, and every element between it and a card back keeps `transform-style: preserve-3d`, so all seats are projected by one camera rather than each faking its own depth. Yaw follows the table rather than the viewer: a player holds their cards face-toward-themselves, so the backs point at whoever sits opposite. North sits opposite the viewer and shows its backs square to the camera. Seats the viewer sits beside show no cards at all. Their hands point across the table, which leaves them edge-on and unreadable from here, so those seats collapse to a name chip placed in the gutter outside the rail at the table's widest point; the felt carries an inline inset to leave that gutter free. Seats angled across the table still hold cards and use `calculateSideFanLayout`, which splays radially about the grip rather than translating along a line -- the head-on calculator collapses into a slab once a seat turns away -- and derives its own card spacing from the per-card bow, because neighbouring cards at different bow angles pinch together toward their edges and would otherwise slice through one another. Played cards, stock cards and discard piles carry the felt's own rotateX so they lie in the table plane instead of standing up in front of it, while status text and prompts stay screen-facing. Because a CSS filter forces a subtree back to flat, opponent hands carry their shadow on the cards.
