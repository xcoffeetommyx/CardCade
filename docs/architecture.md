...
`shared/card-presentation.js` resolves clockwise player order into named table slots relative to the local viewer. The local player is always south; one opponent uses north, two opponents occupy west and north with east left empty, and three opponents use west, north, and east. Opponent hands contain one privacy-safe card-back element for every projected hidden card and use the existing adaptive fan calculator, then hang that fan in the scene's own 3D space. The scene owns a single perspective, and every element between it and a card back keeps `transform-style: preserve-3d`, so all four seats are projected by one camera rather than each faking its own depth. A seat orients its hand with yaw, pitch, and roll: yaw turns the fan to face across the table, so west and east always carry mirrored signs and side hands foreshorten and converge for real. Because a CSS filter forces a subtree back to flat, opponent hands carry their shadow on the cards instead. The interactive local hand keeps its original DOM, fan layout, hit testing, selection, focus, touch, keyboard, and controller behavior.
