...
test("health, catalog, and launcher are served from one process", async (t) => {
  ...
  const localStyles = await fetch(`${origin}/cardcade/app.css?v=136`);
  assert.match(localStyles.headers.get("content-type"), /text\/css/);
  const localApp = await fetch(`${origin}/cardcade/app.js?v=99`);
...
test("the Funnel subpath serves the complete HTTP and WebSocket application", async (t) => {
  ...
  const [styles, appScript, manifest, sharedRules, catalog] = await Promise.all([
    fetch(`${origin}/cardcade/app.css?v=57`),
    fetch(`${origin}/cardcade/app.js?v=58`),
