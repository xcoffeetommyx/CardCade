...
import { MatchEngine } from "../server/src/games/finders-makers/match-engine.js";
import { FindersMakersRuntime } from "../server/src/games/finders-makers/runtime.js";

const runtime = () => new FindersMakersRuntime({ matchEngine: new MatchEngine({ randomIndex: () => 0 }) });
...
test("the Finders Makers runtime maps private game actions and preserves host-only round progression", () => {
  ...
  const opening = game.view(hostRoom);
  ...
  game.act(hostRoom, { type: "finders_search", position: 0 });
  const hostView = game.view(hostRoom);
  const guestView = game.view(roomFor(1));
  ...
  assert.throws(
    () => game.act(roomFor(1), { type: "finders_next_round" }),
...
test("the Finders Makers runtime adds a private-memory CPU to a Solo duel", () => {
  ...
  game.act(room, { type: "finders_search", position: 0 });
  assert.equal(opening.state.board[0].pieceId, undefined);
  assert.equal(game.runBotTurn(room.code), true);

  const afterBotSearch = game.view(room);
